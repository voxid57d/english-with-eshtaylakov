...
async function loadStructuredTest(testId: string) {
   const { data: test, error: testError } = await supabaseAdmin
      .from("reading_mock_tests")
      .select("id, slug, title, description, is_premium, is_published, created_at")
      .eq("id", testId)
      .maybeSingle();
   if (testError || !test) throw new Error("Reading mock test not found.");
   ...
   if (passagesError) throw new Error("Failed to load passages.");
   ...
   if (blocksError) throw new Error("Failed to load blocks.");
   ...
   const { data: options, error: optionsError } =
      questionIds.length === 0
         ...
         : await supabaseAdmin
              ...
              .order("order_index", { ascending: true });
   if (optionsError) throw new Error("Failed to load options.");

   return { ...test, passages: passages || [], blocks: blocks || [], questions: questions || [], options: options || [] };
}
...
export async function PATCH(
   ...
   try {
      ...
      if (!testId || !title || !slug) {
         throw new Error("Test id, title, and slug are required.");
      }
      ...
      const { error: deletePassagesError } = await supabaseAdmin
         .from("reading_mock_passages")
         .delete()
         .eq("test_id", testId);
      ...
      const test = await loadStructuredTest(testId);
      return NextResponse.json({ test });
   } catch (error) {
      return jsonError(error, "Failed to update reading mock test.");
   }
}
...
export async function DELETE(
   ...
   try {
      ...
      const { error } = await supabaseAdmin
         ...
         .delete()
         .eq("id", testId);
      if (error) throw new Error(error.message || "Failed to delete test.");
      return NextResponse.json({ ok: true });
   } catch (error) {
      return jsonError(error, "Failed to delete reading mock test.");
   }
}
