...
export default function MarketingMetrics() {
   ...
   const effectiveEntries = useMemo(() => {
      ...
      for (const [key, draft] of Object.entries(drafts)) {
         if (!draft.entry_date.startsWith(month)) continue;
         try {
   ...
   async function saveEntries() {
      ...
      try {
         ...
         setData((previous) => {
            ...
            changes.filter((change) => change.entry_date.startsWith(month)).forEach((change) => {
   ...
   return <div className={styles.workspace}>
      ...
      {!ready ? <div className={styles.loading}>{loading ? "Loading your marketing workspace…" : "Marketing metrics could not be loaded."}</div> : <>
         ...
         <section className={styles.stats} aria-label="Workspace summary"><div><span>Learning centres</span><strong>{String(data.centres.length).padStart(2, "0")}</strong><small>Your competitive landscape</small></div><div><span>Social platforms</span><strong>{String(data.platforms.length).padStart(2, "0")}</strong><small>Channels you are tracking</small></div><div><span>Recorded on {date.slice(5)}</span><strong>{countsOnDate.length}<em> / {possible}</em></strong><small>Centre and platform pairs</small></div><div><span>Monthly observations</span><strong>{effectiveEntries.length.toLocaleString("en-US")}</strong><small>{draftCount ? "Includes your unsaved preview" : monthLabel}</small></div></section>
